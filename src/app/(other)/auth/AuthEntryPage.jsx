    ...
    <AuthLayout>
      <div className="p-4">
        ...
        <input
          ...
          maxLength={10}
        />
        <button className="btn btn-primary w-100 mb-3" disabled={loading} onClick={handleMobileSubmit}>
          {loading ? 'Sending...' : 'Continue'}
        </button>
        <div id="recaptcha-container"></div>
        {err && <div className="text-danger mb-3">{err}</div>}
        <hr />
        <button className="btn btn-outline-secondary w-100" onClick={() => navigate('/auth/email-login')}>
          Sign in with Email
        </button>
      </div>
    </AuthLayout>
