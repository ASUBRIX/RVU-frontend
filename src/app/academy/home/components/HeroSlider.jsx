    ...
    <section className="hero-slider-section mb-0 pb-0 pt-0 mt-0">
      <TinySlider settings={sliderSettings} className="hero-slider">
        {slides.map((slide, idx) => {
          const imgUrl = slide.image_url.startsWith('http') ? slide.image_url : `${import.meta.env.VITE_API_BASE_URL}${slide.image_url}`
          return (
            <div key={slide.id || idx} className="hero-slide-overlay">
              ...
              <div className="slide-caption-overlay">
                ...
                {slide.link && (
                  <a href={slide.link} target="_blank" rel="noopener noreferrer" className="hero-btn btn btn-lg px-5 py-2">
                    Get Started
                  </a>
                )}
              </div>
